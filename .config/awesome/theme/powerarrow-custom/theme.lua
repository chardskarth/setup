...
local theme                                     = {}
theme.dir                                       = os.getenv("HOME") .. "/.config/awesome/theme/powerarrow-custom"
...
local cpu = lain.widget.cpu({
    settings = function()
        local cpuUsage = cpu_now.usage
        local padded = string.format("%3s", cpuUsage)
        widget:set_markup(markup.font(theme.font, padded .. "% "))
    end
})
...
local net = lain.widget.net({
    settings = function()
        local netReceived = string.format("%6s", string.format("%03.2f", net_now.received))
        local netSent = string.format("%6s", string.format("%03.2f", net_now.sent))
        widget:set_markup(markup.fontfg(theme.font, "#FEFEFE", " ↑ " .. netReceived .. " ↓ " .. netSent .. " "))
    end
})
...
local arrl_tail = separators.arrow_left("alpha", theme.bg_focus)
local function container_alpha (icon, w)
...
function theme.at_screen_connect(s)
   ...
   s.quake = lain.util.quake({ app = "termite", height = 0.50, argname = "--name %s" })
    ...
    s.mytaglist = awful.widget.taglist {
        ...
        update_function = awful.util.taglist_list_update
    }

    -- Create a cool shape for the systray
    ...
    -- Create the wibox
    s.mywibox = awful.wibar({ position = "top", screen = s, height = 20, bg = theme.bg_normal .. "77", fg = theme.fg_normal })
